...
export default function TripResults() {
  ...
  useEffect(() => {
    ...
    if (!data) {
      ...
      if (storedTripData) {
        data = JSON.parse(storedTripData);
      }
    } else {
      sessionStorage.setItem("tripData", JSON.stringify(state));
    }
    setTripData(data);
  }, [state]);

  const { destination, days, startDate, budget, travelType } = tripData || {};
  ...
  const fetchNearbyPlaces = async (city) => {
    try {
      ...
      const elements = data.elements.map((el) => ({
        ...
        lon: el.lon,
        description:
          "A must-visit spot in this city — perfect for sightseeing, food, or culture.",
      }));
  ...
  const getCategoryIcon = (type) => {
    ...
    if (type.includes("place_of_worship"))
        ...
        {itinerary.length > 0 && (
          ...
          >
            ...
            {itinerary.map((day) => (
              <div
                key={day.day}
                className="mb-6 bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition"
              >
